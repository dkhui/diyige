    ...
    public class Tools
    {
        ...
        public static string PostHttpData(string url, string data, string mothod, string cert, string pwd)
        {
            try
            {
                ...
                Debug.WriteLine("Response(" + id + ")：" + rspstr);
                return rspstr;
            }
            catch (Exception ex)
            {
        ...
        public static string PostHttpFile(string url, byte[] data, string filename)
        {
            ...
            try
            {
                ...
                {
                    wc.Headers.Add("UserAgent", HttpContext.Current.Request.UserAgent);
                    wc.Headers.Add("Referer", HttpContext.Current.Request.UrlReferrer.AbsolutePath);
                }
        ...
        public static string PostHttpData(string url, string mothod, Dictionary<string, string> parms)
        {
            ...
            try
            {
                ...
                var id = Guid.NewGuid();
                Debug.WriteLine(mothod + "(" + id + ")：" + url);
                Debug.WriteLine(Serialize.ToJson(parms));
                ...
                Debug.WriteLine("Response(" + id + ")：" + str_back);
                return str_back;
            }
        ...
        public static string GetHttpData(string url, Encoding encd)
        {
            ...
            try
            {
                ...
                //{
                //    wc.Headers.Add("UserAgent", HttpContext.Current.Request.UserAgent);
                //    if (HttpContext.Current.Request.UrlReferrer != null) wc.Headers.Add("Referer", HttpContext.Current.Request.UrlReferrer.AbsolutePath);
                //}
                var id = Guid.NewGuid();
                Debug.WriteLine("GET(" + id + ")：" + url);
                wc = new WebClient();
                sr = new StreamReader(wc.OpenRead(url), encd);
                var data = sr.ReadToEnd();
                Debug.WriteLine("Response(" + id + ")：" + data);
                return data;
            }
            catch (Exception ex)
            {
        ...
        public static string DownImage(string url)
        {
            ...
            catch (Exception ex) { Loger.Error("文件下载失败，错误：" + ex.Message + "\r\n" + ex.StackTrace); return ""; }
        }

        public static string DownImage(string url, string ex)
        {
            try
            {
                var wc = new WebClient();//""
                ...
                var di = new DirectoryInfo(c.Server.MapPath(path));
                if (!di.Exists) di.Create();
                var fname = Guid.NewGuid().ToString() + ex;
                var ms = new MemoryStream();
                ms.Write(data, 0, data.Length);
                var img = Image.FromStream(ms);
                fname = fname + getpix(img.RawFormat);
