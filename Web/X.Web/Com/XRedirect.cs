...
namespace X.Web.Com
{
    public class XRedirect : IHttpModule, IReadOnlySessionState, IRequiresSessionState
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            var hc = (sender as HttpApplication).Context;
            string url = hc.Request.FilePath;

            try
            {
                if (!url.EndsWith(".x") && File.Exists(hc.Server.MapPath(url)) && !url.Contains("/upload/")) { return; }
            }
            catch { }
            ...
            foreach (var r in url_rules.Keys)
            {
                ...
                if (m.Success)
                {
                    newurl = GetNewUrl(m, url_rules[r], hc.Request.QueryString.ToString());
                    break;
                }
            }
            if (!string.IsNullOrEmpty(newurl))
            {
                if (File.Exists(hc.Server.MapPath(newurl.Split('?')[0])))
                    hc.RewritePath(newurl);
            }
        }

        string GetNewUrl(Match m, string url, string query)
        {
