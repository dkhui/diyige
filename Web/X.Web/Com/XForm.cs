...
namespace X.Web.Com
{
    public class XForm
    {
        ...
        public delegate Dictionary<string, string> GetListDataHandler(string cd, string up);
        public static event GetListDataHandler GetListData;
        ...
        static string loadcts(string body)
        {
            ...
            return ctreg.Replace(body, o =>
            {
                ...
                ct.init("");
                Debug.WriteLine(o.Groups[1].Value + "." + o.Groups[2].Value + "->" + (DateTime.Now - dt).TotalMilliseconds);
                return ct == null ? "" : ct.ToString();
            });
        }

        static string getDataName(string cd, string val)
        {
            var dir = GetListData(cd, "00");
        ...
        class text : ct, ict
        {
            ...
            {
                if (string.IsNullOrEmpty(val)) val = def;
                var tipstr = string.IsNullOrEmpty(tip) ? title : tip;
                if (tp == 1)
                    return "<input type='text' id='id_" + name + "' name='" + name + "' x-check='" + c.ToString() + "' title='" + tipstr + "' placeholder='" + tipstr + "' class='input-" + sizes[size - 1] + "' value='" + val + "' />";
                else if (tp == 2)
                    return "<input type='password' id='id_" + name + "' name='" + name + "' title='" + tipstr + "' x-check='" + c.ToString() + "' placeholder='" + tipstr + "' class='input-" + sizes[size - 1] + "' value='" + val + "' />";
                else if (tp == 3)
                    return string.Format("<textarea id='id_{0}' name='{0}' placeholder='{1}' x-check='{2}' class='input-{3}' title='" + tipstr + "' >{4}</textarea>",
                             name,
                             tipstr,
                             c.ToString(),
                             sizes[size - 1],
        ...
        class pick : ct, ict
        {
            ...
            public void init(string body)
            {
                ...
                if (string.IsNullOrEmpty(val)) val = def + "";// val.Trim();
                val = val.Trim();
                ...
                if (mode == 2 && (data[0] == "dict" || data[0] == "view" || data[0] == "loc"))
                {
                    sb_html.Append("<div class='btn-group' title='" + title + "' x-check='" + (no == 1 ? "{\"no\":true}" : "") + "' x-val='" + val + "' name='" + name + "'>");
                    if (data[0] == "dict" || data[0] == "view")
                    {
                        var d = GetListData(data[1], upid);
                        foreach (var i in d) sb_html.Append("<span class='btn " + (val == i.Key || (val != null && val.Contains("[" + i.Key + "]")) ? "btn-primary" : "") + "' name='" + name + "' x-val='" + i.Key + "'>" + i.Value + "</span>");
                    }
                    else
                    {
                        ...
                        foreach (var p in st)
                        {
                            ...
                            sb_html.Append("<span class='btn " + (val == _v || (val != null && val.Contains("[" + _v + "]")) ? "btn-primary" : "") + "' name='" + name + "' x-val='" + _v + "'>" + _p[0] + "</span>");
                        }
                    }
                    sb_html.Append("</div>");
                }
                ...
                {
                    var n = getDataName(data[1], val);
                    sb_html.Append("<span class='btn' x-to='" + to + "' " + (string.IsNullOrEmpty(upid) ? "" : "x-parms='" + upid + "'") + " x-src='" + src + "' name='" + name + "' x-val='" + val + "'     ='" + title + "' x-check='" + (no == 1 ? "{\"no\":true}" : "") + "' title='" + (string.IsNullOrEmpty(tip) ? "请选择 " + title : tip) + "' onclick='x.pick.show(this)'><span class='text' txt=''>" + (string.IsNullOrEmpty(n) ? "请选择" : n) + "</span><i class='icon-caret-down'></i></span>");
                }
                else
                {
                    sb_html.Append("<span class='btn' x-to='" + to + "' " + (string.IsNullOrEmpty(upid) ? "" : "x-parms='" + upid + "'") + " x-src='" + src + "' name='" + name + "' x-val='' title='" + title + "' x-check='" + (no == 1 ? "{\"no\":true}" : "") + "' title='" + (string.IsNullOrEmpty(tip) ? "请选择 " + title : tip) + "' onclick='x.pick.show(this)'><span class='text' txt=''>请选择</span><i class='icon-caret-down'></i></span>");
                }
        ...
        class upload : ct, ict
        {
            ...
            /// 其它参数
            /// </summary>
            public string exparms { get; set; }

            public void init(string body)
            {
                sb_html.Append("<div class='li upload " + name + " ' id='li_" + name + "' x-tp='" + tp + "' x-count='" + (count == 0 ? 1 : count) + "' x-name='" + name + "' x-exparms='" + exparms + "' x-upapi='" + upapi + "' x-delapi='" + delapi + "' x-enc='" + enc + "'>");
                if (!string.IsNullOrEmpty(title)) sb_html.Append("<label class='lbe' for='id_" + name + "'>" + (no == 1 ? "<span class='star'>*</span>" : "") + title + "：</label>");
                sb_html.Append("<span class='btn xbtn'><i class='icon-upload-alt'></i>上传文件</span><input type='hidden' name='" + name + "' id='id_" + name + "' title='" + title + "' x-check='" + (no == 1 ? "{\"no\":true}" : "") + "' value='" + val + "' /><div class='" + (tp == "img" ? "imgs" : "files") + "'></div>");
                sb_html.Append("</div>");
            }
