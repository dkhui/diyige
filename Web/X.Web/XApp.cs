...
namespace X.Web
{
    public class XApp : IHttpHandler, IRequiresSessionState
    {
        public void ProcessRequest(HttpContext c)
        {
            ...
            Loger.Info(String.Format("X.App.{0}.{1}", (t == 1 ? "Views" : "Apis"), v));
            try
            {
            ...
            catch (XExcep fe)
            {
                ...
                if (t == 1)
                {
                    ...
                    view.dict.Add("backurl", Secret.ToUrlBase64(c.Request.RawUrl));
                ...
                foreach (string k in c.Request.Params.Keys) { if (c.Request.ServerVariables.AllKeys.Contains(k)) continue; ds.Add(k, c.Request.Params[k]); }
                Loger.Error(Serialize.ToJson(ds));
            }
            catch (Exception ex)
            {
