...
(function ($) {
    ...
    var form = function (el, op) {
        ...
        f.vt = {
            ...
            url: { reg: /^([hH][tT]{2}[pP]:\/\/|[hH][tT]{2}[pP][sS]:\/\/)(([A-Za-z0-9-~]+)\.)+([A-Za-z0-9-~\/])+([^\x00-\xff])*$/, msg: "不是有效的Url格式，例：http://www.80xc.com！" },
    ...
    function loadcontent(d) {
        switch (d[0]) {
            ...
            case "url":
                ...
                var u = "/com/view-" + d[1] + "-{key}-" + (pick.ct.attr("x-parms") || "");
...
(function ($) {
    ...
    var upload = {
        init: function () {
            ...
            $(".upload").each(function () {
                var up = $(this);
                var exps = up.attr("x-exparms");
                if (exps) exps = exps.replace(/:/g, "=").replace(/,/g, "&");
                ...
                for (var i in vs) {
                    var li = null;
                    var f = vs[i];
                    if (tp == "img") li = $("<div class='img' x-val='" + f + "'><img src='" + f + "?400' /></div>");
                    else {
                        var fs = f.split('|');
                        li = $("<div class='file' x-val='" + f + "'><img src='/img/files/" + fs[2] + ".png' /><p class='attr'><span class='n'>" + fs[1] + "</span>" + fs[3] + "<a href='" + fs[0] + "?" + exps + "'>查看</a ></p ></div > ");
                    }
                    list.append(li);
                }
        ...
        del: function (obj) {
            var li = $(obj).parent();
            x.confirm("确定删除文件么，文件将不能恢复！", function () {
                ...
                form.append("url", url);
                var exps = ipt.parent().attr("x-exparms").split(',');
                for (var p in exps) {
                    var ep = exps[p].split(":");
                    if (ep.length != 2) continue;
                    form.append(ep[0], ep[1]);
                }
                ...
                xhr.open("post", ipt.parent().attr("x-delapi") || "/api/com.remove", true);
                xhr.send(form);
            });
        },
        up: function (file) {
            ...
            $("#x-upload-file").val("");
            function send(li, ipt) {
                var form = new FormData();
                form.append("type", upload.tp);
                form.append("file", f);// 文件对象
                form.append("enc", list.parent().attr("x-enc"));
                var exps = ipt.parent().attr("x-exparms").split(',');
                for (var p in exps) {
                    var ep = exps[p].split(":");
                    if (ep.length != 2) continue;
                ...
                xhr.onreadystatechange = function () {
                    if (this.readyState != 4 || this.status != 200) return;
                    var j = jQuery.parseJSON(this.responseText);
                    if (!j.issucc) { if (j.msg) alert(j.msg); li.remove(); return; }

                    if (upload.tp == "img") {
                        li.attr("x-val", j.url);
                        li.find("img").attr("src", j.url + "?400");
                    }
                    else {
                        li.attr("x-val", j.url + "|" + j.name + "|" + j.ext + "|" + getsize(j.size));
                        li.find("img").attr("src", "/img/files/" + j.ext + ".png");
                        li.find("span.n").text(j.name);
                    }
                    setval(li.parent(), ipt);
                }
                xhr.open("post", ipt.parent().attr("x-upapi") || "/api/com.upload", true);
                xhr.send(form);
            }
        },
        doup: function (tp, ct) {
            if (!tp) { console.log("上传类型为空，请设置控件tp属性"); return; }
            upload.ct = ct;
            upload.tp = tp;
    ...
    function getsize(s) {
        ...
        s = s / 1000;
        if (s < 1000) return s.toFixed(2) + "Gb";
        s = s / 1000;
        if (s < 1000) return s.toFixed(2) + "Tb";
        s = s / 1000;
