x.grid = function (cfg) {
    ...
    this.getlist = function (d, callback) {
        var g = this;
        $(g.config.el + " .btn.pi").html("<span class='loading'>数据加载中...</span>");
    ...
    function renderpager(g, d) {
        ...
        if (cfg.searchcon.limit) p.find(".pi").html(pz + "条/页，" + "共" + d.count + "条，当前" + d.page + "页/共" + pc + "页");
        else p.find(".pi").html("共有" + d.count + "条记录");

        if (d.count <= pz) gDom.find(".pull-right.pagination").hide();
        else { gDom.find(".pull-right.pagination").show(); }

        if (cfg.searchcon.limit) {
    ...
    function rendertable(body, d) {
        ...
        for (var i = 0; i < d[cfg.listvar].length; i++) {
            ...
            for (var x = 0; x < cfg.fs.length; x++) {
                ...
                val = val.replace(/{([\w_-\d.]+)}/g, function (all, w) {
                    w = w.split('.');
                    if (w.length > 2) return "";
                    else if (w.length == 1) return row[w] || "";
                    else return row[w[0]] == undefined ? "" : row[w[0]][w[1]] || "";
                ...
                if (fe.tpl) {
                    ...
                    var link = fe.link.replace(/{([\w_-\d]+)}/g, function (all, w) { return row[w] || ""; });
                    var text = link.replace(/([^\x00-\xff]+)/g, function (all, w) { return escape(escape(all)) });
                    td.append("<a href='" + text + "' target='" + fe.target + "'>" + (val || "点击打开") + "</a>");
                } else if (fe.act) {
    ...
    function getbtn(b, row, i) {
        ...
        if (b.act) btn.data("confirm", b.confirm == undefined ? true : b.confirm).attr("data-id", i).attr("data-act", b.act).css("cursor", "pointer");
            ...
            var link = b.link.replace(/{([\w_-\d]+)}/g, function (all, w) { return row[w] || ""; });
            var text = link.replace(/([^\x00-\xff]+)/g, function (all, w) { return escape(escape(w)) });
    ...
    function rendertpl(g, d) {
        for (var i = 0; i < d[cfg.listvar].length; i++) {
            ...
            tpl = tpl.replace(/{{(([\S ](?!{{))+)}}/ig, function (all, tp) {
                ...
                for (var i in _items) {
                    var _da = _items[i];
                    sbtp += tps[1].replace("{_count}", _items.length).replace("{_index}", i).replace(/{([\w\d]+)}/ig, function (all, s1) { return _da[s1]; });
                }
                return sbtp;
            });
    ...
    function render(d) {
        ...
        if (cfg.tpl) {
            gDom.find(".grid.head").hide();
            rendertpl(g, d);
        } else {
            gDom.find(".grid.head").html(gettablehead());
            rendertable(g, d);
        }
