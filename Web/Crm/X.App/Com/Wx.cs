    ...
    public class Wx
    {
        public static string GetToken(string appid, string sec, bool isnew)
        {
            ...
            if (string.IsNullOrEmpty(tk) || isnew)
            {
                ...
                tk = tke.access_token;
                CacheHelper.Save("wx.access_token", tk, tke.expires / 2);
            }
            else
            {
                var ips = GetWxIp(tk);
                if (ips == null) tk = GetToken(appid, sec, true);
            }
            return tk;
        }
        ...
        public static string[] GetWxIp(string tk)
        {
            var json = Tools.GetHttpData("https://api.weixin.qq.com/cgi-bin/getcallbackip?access_token=" + tk);
            var rsp = Serialize.FromJson<ips>(json);
            if (!string.IsNullOrEmpty(rsp.errcode)) return null;
            return rsp.ip_list;
        }
        ...
        public class Web
        {
            public static token GetToken(string appid, string sec, string code)
            {
            ...
            public static string GetJsTicket(string tk, bool isnew)
            {
                ...
                if (string.IsNullOrEmpty(tick) || isnew)
                {
                    ...
                    tick = tkn.ticket;
                    CacheHelper.Save("wx.js_ticket", tick, tkn.expires_in / 2);
                }
                return tick;
            }
        ...
        public class Pay
        {
            ...
            /// </param>
            /// <returns></returns>
            public static Oxml MdOrder(string body, string order_no, string total, string notify_url, string openid, string appid, string mch_id, string mch_key, int pl)
            {
                return MdOrder(body, order_no, total, notify_url, openid, appid, mch_id, mch_key, pl, null);
            }

            public static Oxml MdOrder(string body, string order_no, string total, string notify_url, string openid, string appid, string mch_id, string mch_key, int pl, string ip)
            {
        ...
        public class Msg
        {
            ...
            public static bool SendTpl(string tk, string opid, string tplid, string url, Dictionary<string, Tplmsg> ps)
            {
                ...
                data.Append("}}");
                var back = Serialize.FromJson<mbase>(Tools.PostHttpData(api, data.ToString()));
        ...
        public class WxExcep : Exception
        {
            ...
            public WxExcep(string msg)
                : base(msg)
            {
                if (msg.IndexOf("{") == 0) error = Serialize.FromJson<Err>(msg);
